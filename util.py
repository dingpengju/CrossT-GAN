...
def metrics_calculate(values, re_values, labels):
    ...
    preds, _ = evaluate(labels, scores, adj=True)
    ...
    print('F1 score: %.5f, AUC: %.5f.' % (f1, auc))
    print('Precision: %.5f, Recall: %.5f.' % (pre, re))
...
def load_data(data_prefix, val_size, window_size=120, stride=1, batch_size=32, dataloder=False):
